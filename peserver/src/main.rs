...
async fn send_response(mut socket: TcpStream, status: usize, message: &str) {
...
async fn process_api_v1_i(mut socket: TcpStream, content_length: usize, mut body_start: Vec<u8>) {
    ...
    let archive_size = content_length - config_size;  // checked above
    ...
    eprintln!("got api data {api_v1_data:?}");
...
async fn process1(mut socket: TcpStream, addr: SocketAddr) {
